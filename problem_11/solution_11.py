...
def table_conversion_in_list_of_list(file="greed.txt") -> list:
    with open(file) as file:
        tbl = file.read()
    res = []
    tbl = tbl.split("\n")
    for i, row in enumerate(tbl):
        res.append([int(elem)for elem in row.split(" ")])
    return res


@timer
def problem_11(n=4):
    tbl = table_conversion_in_list_of_list()
    seq = list(range(n))
    res_lst = []
    if n > len(tbl):
        raise ValueError("Длина последовательности больше размера таблицы")
    for i in range(len(tbl)):
        for j in range(0, len(tbl[i]) - n + 1):
            mul = 1
            for k in seq:
                mul *= tbl[i][j + k]
            res_lst.append(mul)
    for i in range(len(tbl) - n + 1):
    ...
    for i in range(len(tbl) - n + 1):
        for j in range(0, len(tbl[i]) - n + 1):
            mul = 1
            for ind, k in enumerate(seq):
                mul *= tbl[i + k][j + seq[-1-ind]]
            res_lst.append(mul)
    return max(res_lst)
